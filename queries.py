...
def Q_1(conn, execution_time):
    ...
    query = """ select shot.player_name, AVG(shot.statsbomb_xg) as avg_score
                from competitions
                join matches on competitions.competition_id = matches.competition_id and competitions.season_id = matches.season_id
                join shot on matches.match_id = shot.match_id
                WHERE competitions.competition_name = 'La Liga' AND competitions.season_name = '2020/2021'
                GROUP BY (shot.player_id, shot.player_name)
...
def Q_2(conn, execution_time):
    ...
    query = """SELECT shot.player_name, COUNT(*) as shot_count
                from competitions
                join matches on competitions.competition_id = matches.competition_id and competitions.season_id = matches.season_id 
                join shot on matches.match_id = shot.match_id
                WHERE competitions.competition_name = 'La Liga' AND competitions.season_name = '2020/2021'
...
def Q_3(conn, execution_time):
    ...
    query = """select shot.player_name, COUNT(*) as first_time_shots
                from competitions
                join matches on competitions.competition_id = matches.competition_id and competitions.season_id = matches.season_id 
                join shot on matches.match_id = shot.match_id
                WHERE (competitions.competition_name = 'La Liga') 
...
def Q_4(conn, execution_time):
    ...
    query = """select pass.team_name, COUNT(*) as num_passes
                from competitions
                join matches on competitions.competition_id = matches.competition_id and competitions.season_id = matches.season_id 
                join pass on matches.match_id = pass.match_id
                WHERE competitions.competition_name = 'La Liga'
				AND competitions.season_name = '2020/2021'
                GROUP BY (pass.team_id, pass.team_name)
...
def Q_5(conn, execution_time):
    ...
    query = """select players.player_name, COUNT(*) as num_recipiencies
                from competitions
                join matches on competitions.competition_id = matches.competition_id and competitions.season_id = matches.season_id 
                join pass on matches.match_id = pass.match_id
                join players on players.player_id = pass.recipient_id
                WHERE competitions.competition_name = 'Premier League'
				AND competitions.season_name = '2003/2004'
                GROUP BY (players.player_id)
                ORDER BY (num_recipiencies) DESC """
...
def Q_6(conn, execution_time):
    ...
    query = """ select shot.team_name, COUNT(*) as most_shots
                from competitions
                join matches on competitions.competition_id = matches.competition_id and competitions.season_id = matches.season_id 
                join shot on matches.match_id = shot.match_id
                WHERE competitions.competition_name = 'Premier League'
				AND competitions.season_name = '2003/2004'
                GROUP BY (shot.team_id, shot.team_name)
                ORDER BY (most_shots) DESC"""
...
def Q_7(conn, execution_time):
    ...
    query = """ select pass.player_name, COUNT(*) as most_through_balls
                from competitions
                join matches on competitions.competition_id = matches.competition_id and competitions.season_id = matches.season_id 
                join pass on matches.match_id = pass.match_id
                WHERE competitions.competition_name = 'La Liga'
                ...
                GROUP BY (pass.player_id, pass.player_name)
                ORDER BY (most_through_balls) DESC"""
...
def Q_8(conn, execution_time):
    ...
    query = """select pass.team_name, COUNT(*) as most_through_balls
                from competitions
                join matches on competitions.competition_id = matches.competition_id and competitions.season_id = matches.season_id 
                join pass on matches.match_id = pass.match_id
                WHERE competitions.competition_name = 'La Liga'
...
def Q_9(conn, execution_time):
    ...
    query = """ select dribble.player_name, COUNT(*) as most_completed_dribbles
                from competitions
                join matches on competitions.competition_id = matches.competition_id and competitions.season_id = matches.season_id 
                join dribble on matches.match_id = dribble.match_id
                WHERE (competitions.competition_name = 'La Liga') 
				AND (competitions.season_name = '2020/2021'
				    OR competitions.season_name = '2019/2020'
				    OR competitions.season_name = '2018/2019')
				AND outcome = 'Complete'
                GROUP BY (dribble.player_id, dribble.player_name)
                ORDER BY (most_completed_dribbles) DESC """
...
def Q_10(conn, execution_time):
    ...
    query = """ select dribbledpast.player_name, COUNT(*) as least_dribbled_past
                from competitions
                join matches on competitions.competition_id = matches.competition_id and competitions.season_id = matches.season_id 
                join dribbledpast on matches.match_id = dribbledpast.match_id
                WHERE (competitions.competition_name = 'La Liga') 
				AND competitions.season_name = '2020/2021'
                GROUP BY (dribbledpast.player_id, dribbledpast.player_name)
                ORDER BY (least_dribbled_past) ASC"""
